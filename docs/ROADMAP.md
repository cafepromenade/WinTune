...
- [x] **Human-readable + settable outputs** · 可讀＋可設定 — DONE (core): new `Tweak.Table(...)` factory runs a PowerShell pipeline, appends `ConvertTo-Csv` (with `$ProgressPreference='SilentlyContinue'` so CLIXML never leaks), and `TweakCard` renders the CSV as a **native, selectable column grid** (RFC-4180 parser that drops ragged/noise rows) instead of a monospace dump. Converted the Network Pro "Show/List" adapter tweaks (list, statistics, IP addresses, advanced props, bindings, link speed). _Follow-ups: sweep remaining catalog "Show …" actions to `Tweak.Table`; add per-row actions (enable/disable adapter) on table rows._
